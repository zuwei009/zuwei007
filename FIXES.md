...
### 更新内容 / Updates
根据实际情况优化了硅钼棒的显示方式：
- **移除了棒方向选择**：硅钼棒实际上是垂直安装的
- **俯视图显示**：在炉膛俯视图中，垂直安装的硅钼棒显示为红色圆点（横截面）
- **简化配置**：移除了 `orientation` 属性，简化了用户界面

### 技术变更 / Technical Changes
1. 移除了 `RodConfiguration` 接口中的 `orientation` 属性
2. 简化了 `drawHeatingRods` 函数，只保留圆点绘制逻辑
3. 移除了UI中的"棒方向"选择器
4. 更新了文档说明，明确硅钼棒是垂直安装的
